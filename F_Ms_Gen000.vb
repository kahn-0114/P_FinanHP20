...
Public Class F_Ms_Gen000
    ...
    Public Sub Data_Get00()
        ...
        With dt0.Tables("t0")
            If .Rows.Count > 0 Then
                txt見出1.Text = PubCom.ChgNull(.Rows(0)("会社名"), 1)
                txt1.Text = PubCom.ChgNull(.Rows(0)("会社名Kana"), 1)

                txt2.Text = PubCom.ChgNull(.Rows(0)("郵便番号1"), 1)
                txt郵便番号2.Text = PubCom.ChgNull(.Rows(0)("郵便番号2"), 1)
                txt3.Text = PubCom.ChgNull(.Rows(0)("住所1"), 1)
                txt住所2.Text = PubCom.ChgNull(.Rows(0)("住所2"), 1)
                txt4.Text = PubCom.ChgNull(.Rows(0)("電話番号"), 1)
                txt決済1.Text = PubCom.ChgNull(.Rows(0)("FAX番号"), 1)
                txt代表役職.Text = PubCom.ChgNull(.Rows(0)("代表者役職"), 1)
                txt代表者名.Text = PubCom.ChgNull(.Rows(0)("代表者名"), 1)
                txt5.Text = PubCom.ChgNull(.Rows(0)("決算月"), 1)
                txt科目CD1.Text = PubCom.ChgNull(.Rows(0)("決算日"), 1)
                txt端数計算区分.Text = PubCom.ChgNull(.Rows(0)("端数計算区分"), 1)
                txt科目名１.Text = PubCom.GetGengo(.Rows(0)("消費税実施日"), "yyyy/MM/dd")
                txt科目枝番CD1.Text = PubCom.ChgNull(.Rows(0)("消費税率"), 1)
                txt科目名2.Text = PubCom.ChgNull(.Rows(0)("旧消費税率"), 1)
    ...
    Public Sub Data_Put00S0()
        ...
        command.Parameters.Add("@会社名", SqlDbType.NVarChar).Value = txt見出1.Text
        command.Parameters.Add("@会社名Kana", SqlDbType.NVarChar).Value = txt1.Text
        command.Parameters.Add("@郵便番号", SqlDbType.NVarChar).Value = txt2.Text & "-" & txt郵便番号2.Text
        command.Parameters.Add("@郵便番号1", SqlDbType.NVarChar).Value = txt2.Text
        command.Parameters.Add("@郵便番号2", SqlDbType.NVarChar).Value = txt郵便番号2.Text
        command.Parameters.Add("@住所1", SqlDbType.NVarChar).Value = txt3.Text
        command.Parameters.Add("@住所2", SqlDbType.NVarChar).Value = txt住所2.Text
        command.Parameters.Add("@電話番号", SqlDbType.NVarChar).Value = txt4.Text
        command.Parameters.Add("@FAX番号", SqlDbType.NVarChar).Value = txt決済1.Text
        ...
        command.Parameters.Add("@決算月", SqlDbType.Int).Value = IIf(IsNumeric(txt5.Text), txt5.Text, DBNull.Value)
        command.Parameters.Add("@決算日", SqlDbType.Int).Value = IIf(IsNumeric(txt科目CD1.Text), txt科目CD1.Text, DBNull.Value)
        ...
        If IsDate(txt科目名１.Text) Then
            command.Parameters("@消費税実施日").Value = Date.Parse(txt科目名１.Text).ToString("yyyyMMdd")
        End If
        command.Parameters.Add("@消費税率", SqlDbType.Decimal).Value = IIf(IsNumeric(txt科目枝番CD1.Text), txt科目枝番CD1.Text, 0)
        command.Parameters.Add("@旧消費税率", SqlDbType.Decimal).Value = IIf(IsNumeric(txt科目名2.Text), txt科目名2.Text, 0)
        ...
        Select Case ctxtInp.Name
            ...
            Case "txt会社名"
                ...
                If txt見出1.Text = "" Then
                    ...
                    Inp_Chk00 = False : txtMsg.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                Else
                    Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt見出1.Text)
                    If LenB > 50 Then
            ...
            Case "txt会社Kana名"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt1.Text)
                If LenB > 40 Then
            ...
            Case "txt住所1"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt3.Text)
                If LenB > 100 Then
            ...
            Case "txt電話番号"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt4.Text)
                If LenB > 15 Then
                    txtMsg.Text = "電話番号:15"
                    Inp_Chk00 = False : txtMsg.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                End If
            Case "txtFax番号"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt決済1.Text)
                If LenB > 15 Then
            ...
            Case "txt決算月"
                ctxtInp.BackColor = Color.White
                If txt5.Text = "" Then Exit Function
                ...
                Dim wTmp10 As Byte = Byte.Parse(txt5.Text)
            ...
            Case "txt決算日"
                ctxtInp.BackColor = Color.White
                If txt科目CD1.Text = "" Then Exit Function

                If Not IsNumeric(txt科目CD1.Text) Then
                    txtMsg.Text = "決算日不正"
                    Inp_Chk00 = False : txtMsg.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                End If

                Dim wTmp10 As Byte = Byte.Parse(txt科目CD1.Text)
            ...
            Case "txt消費税実施日"
                ctxtInp.BackColor = Color.White
                If txt科目名１.Text = "" Then Exit Function

                If IsDate(txt科目名１.Text) Then
                    txt科目名１.Text = Date.Parse(txt科目名１.Text).ToString("yyyy/MM/dd")
                    Exit Function
                End If

                Dim w日付 As String = ""
                Dim wResult = PubCom.DateChk00(txt科目名１.Text, w日付)
                If wResult = 1 Then
    ...
    Private Sub La端数区分_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles La科目枝番CD1.Click
    ...
    Private Sub txt決済3_TextChanged(sender As Object, e As EventArgs) Handles txt決済3.TextChanged
