...
Public Class F_Ms_MotGin
    ...
    Public Sub Data_Get00()
        ...
        SQL = "SELECT * " _
            & "From MM_会社00 " _
            & "WHERE (会社No =" & txt科目集計CD.Text & ")"
        SQLCmd.CommandText = SQL
        da0.SelectCommand = SQLCmd
        da0.Fill(dt0, "t0")
        With dt0.Tables("t0")
            If .Rows.Count > 0 Then
                txt銀行名.Text = PubCom.ChgNull(.Rows(0)("会社名"), 1)
                txt口座番号.Text = PubCom.ChgNull(.Rows(0)("会社名Kana"), 1)
    ...
    Public Sub Data_Put00S0()
        ...
        command.Parameters.Add("@会社No", SqlDbType.Int).Value = txt科目集計CD.Text
        command.Parameters.Add("@会社名", SqlDbType.NVarChar).Value = txt銀行名.Text
        command.Parameters.Add("@会社名Kana", SqlDbType.NVarChar).Value = txt口座番号.Text
    ...
    Public Sub Data_Put00S1()
        ...
        SQL = SQL & " WHERE (会社No=" & txt科目集計CD.Text & ")"
        Dim command As SqlCommand = New SqlCommand(SQL, Cn00)
        command.Parameters.Clear()
        ...
        Select Case ctxtInp.Name
            ...
            Case "txt会社名"
                ...
                If txt銀行名.Text = "" Then
                    ...
                    Inp_Chk00 = False : txtMsg.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                Else
                    Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt銀行名.Text)
                    If LenB > 50 Then
            ...
            Case "txt会社Kana名"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt口座番号.Text)
                If LenB > 40 Then
    ...
    Private Sub btnOK00_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim pRet As Boolean = Inp_Chk00(txt科目集計CD)
        If pRet = False Then Exit Sub
        Data_Get00()
        txt銀行名.Focus()
    ...
    Private Sub txtInp_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Select Case pTxtBox.Name
            Case "txt会社No"
                AddHandler txt科目集計CD.TextChanged, AddressOf txtModified_TextChanged
        End Select
        pFocus(0) = pTxtBox
    End Sub
    ...
    Private Sub CmbComp_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            txt科目集計CD.Text = item.ItemCode
            pubComPany = Integer.Parse(txt科目集計CD.Text)
            btnOK01.PerformClick()
        End If
    End Sub
    ...
    Private Sub GroupBox00_Enter(sender As Object, e As EventArgs)
    ...
    Private Sub Label4_Click(sender As Object, e As EventArgs) Handles La預金種別.Click

    End Sub

    Private Sub btn支店保存_Click(sender As Object, e As EventArgs) Handles btn保存.Click

    End Sub

    Private Sub txt会社CD1_TextChanged(sender As Object, e As EventArgs) Handles txt会社CD1.TextChanged
