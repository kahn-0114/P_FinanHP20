...
Public Class F_Mnt_Org00
    ...
    Private Sub Inp_Clr00()
        ...
        For Each CtrlItem As Control In Me.GroupBox10.Controls
            If TypeOf CtrlItem Is TextBox Then CtrlItem.Text = ""
        Next
        txtメッセージ.Text = "" : txtメッセージ.Visible = False
        CmbComp.SelectedIndex = -1
    ...
    Public Sub Data_Put00S0()
        ...
        command.Parameters.Add("@会社No", SqlDbType.Int).Value = txt整理対象日.Text
    ...
    Public Sub Data_Put00S1()
        ...
        SQL = SQL & " WHERE (会社No=" & txt整理対象日.Text & ")"
        Dim command As SqlCommand = New SqlCommand(SQL, Cn00)
        command.Parameters.Clear()
    ...
    Private Function Inp_Chk00(ByVal ctxtInp As Control)
        Inp_Chk00 = True
        txtメッセージ.Visible = False : txtメッセージ.Text = ""
        Select Case ctxtInp.Name
            Case "txt会社No"
                If txt整理対象日.Text = "" Then
                    txtメッセージ.Text = "会社CD未入力"
                    Inp_Chk00 = False : txtメッセージ.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                End If
            Case "txt会社名"
                ctxtInp.BackColor = Color.White
                If txt会社名.Text = "" Then
                    txtメッセージ.Text = "会社名未入力"
                    Inp_Chk00 = False : txtメッセージ.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                Else
                    Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt会社名.Text)
                    If LenB > 50 Then
                        txtメッセージ.Text = "会社名超過:50"
                        Inp_Chk00 = False : txtメッセージ.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                    End If
                End If
            Case "txt会社Kana名"
                ctxtInp.BackColor = Color.White
                Dim LenB As Integer = System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(txt会社Kana名.Text)
                If LenB > 40 Then
                    txtメッセージ.Text = "会社名かな超過:40"
                    Inp_Chk00 = False : txtメッセージ.Visible = True : ctxtInp.BackColor = Color.LightCoral : Exit Function
                End If
            Case "txt住所1"
                ctxtInp.BackColor = Color.White
            ...
            Case "txt消費税実施日"
                ...
                If wResult = 1 Then
                    txtメッセージ.Text = "消費税実施日不正"
                    Inp_Chk00 = False : txtメッセージ.Visible = True : ctxtInp.BackColor = Color.LightCoral
                Else
                    txt消費税実施日.Text = w日付
                End If
                Exit Function
    ...
    '* 会社情報取得
    '******************************************************
    Private Sub btnOK00_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn全て選択.Click
        Dim pRet As Boolean = Inp_Chk00(txt整理対象日)
        If pRet = False Then Exit Sub
        Data_Get00()
        txt会社名.Focus()
    ...
    Private Sub txtInp_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Select Case pTxtBox.Name
            Case "txt会社No"
                AddHandler txt整理対象日.TextChanged, AddressOf txtModified_TextChanged
        End Select
        pFocus(0) = pTxtBox
    End Sub
    ...
    Private Sub CmbComp_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            txt整理対象日.Text = item.ItemCode
            pubComPany = Integer.Parse(txt整理対象日.Text)
            btn全て選択.PerformClick()
        End If
    End Sub
    ...
    Private Sub txt請求年度_TextChanged(sender As Object, e As EventArgs)
