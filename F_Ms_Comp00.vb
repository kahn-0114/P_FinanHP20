...
Public Class F_Ms_Comp00
    ...
    Public Sub Data_Put00S0()
        ...
        command.Parameters.Add("@会社No", SqlDbType.Int).Value = txt会社CD.Text
    ...
    Public Sub Data_Put00S1()
        ...
        SQL = SQL & " WHERE (会社No=" & txt会社CD.Text & ")"
        Dim command As SqlCommand = New SqlCommand(SQL, Cn00)
        command.Parameters.Clear()
    ...
    Private Sub btnOK00_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK00.Click
        Dim pRet As Boolean = Inp_Chk00(txt会社CD)
        If pRet = False Then Exit Sub
        Data_Get00()
        txt会社名.Focus()
    ...
    Private Sub txtInp_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Select Case pTxtBox.Name
            Case "txt会社No"
                AddHandler txt会社CD.TextChanged, AddressOf txtModified_TextChanged
        End Select
        pFocus(0) = pTxtBox
    End Sub
    ...
    Private Sub CmbComp_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            txt会社CD.Text = item.ItemCode
            pubComPany = Integer.Parse(txt会社CD.Text)
            btnOK00.PerformClick()
        End If
    End Sub
    ...
    Private Sub La端数区分_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles La端数計算区分.Click
    ...
    Private Sub La消費税区分_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles La消費税区分.Click
        Dim frmForm = New F_S_KuBunM00
    ...
    Private Sub La消費税計算_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles La消費税計算.Click
        Dim frmForm = New F_S_KuBunM00
    ...
    Private Sub La消費税端数_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles La消費税端数.Click
        Dim frmForm = New F_S_KuBunM00
        frmForm.pubKuCD = 3 : frmForm.pubCD = ""
        If frmForm.ShowDialog() = Windows.Forms.DialogResult.OK Then
            If IsNumeric(frmForm.pubCD) Then
    ...
    Private Sub txt請求年度_TextChanged(sender As Object, e As EventArgs) Handles txt請求年度.TextChanged
